...
It'll show an error in the log if there were any problems while compiling.[fn:3]
...
Finally press =CTRL-s= to save.[fn:2]
** Build
...
[fn:2] There seems to be a bug with Projucer under Archlinux, where Projucer crashes right after finishing the first export job. If that still happens to you and the Makefile wasn't exported correctly, disable/remove all other exporters before saving to ensure the Makefile is exported first.

[fn:3] If there are no errors but the node still sounds bypassed, check that the number of channels in the network and in the Faust code match.
